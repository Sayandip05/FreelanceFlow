...
export default function AuthPage() {
  ...
  // Ensure continuous looping video plays reliably across all devices/browsers
  useEffect(() => {
    if (videoRef.current) {
  ...
  return (
    <div className="min-h-screen w-full bg-white flex">
      {/* ── Spacer: reserves flex space equal to fixed left panel width ──── */}
      <div className="hidden lg:block flex-shrink-0" style={{ width: 'min(820px, 50vw)' }} />

      {/* ── Left Panel: Fixed to viewport — video never reflowed by zoom ─── */}
      <div
        className="hidden lg:block fixed top-0 left-0 h-screen overflow-hidden bg-[#e8f2fc] border-r border-gray-100"
        style={{ width: 'min(820px, 50vw)', zIndex: 10 }}
      >
        {/* Back to Home Button */}
            ...
            <div className="mb-5 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`p-3 rounded-xl border text-left transition-all flex items-center gap-2.5 ${regForm.role === 'FREELANCER'
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border text-left transition-all flex items-center gap-2.5 ${regForm.role === 'CLIENT'
                      ? 'border-blue-600 bg-blue-50/50 ring-2 ring-blue-600/20 text-gray-900'
                      : 'border-gray-200 hover:border-gray-300 text-gray-600 bg-white'
                    }`}
                >
