...
import { Eye, EyeOff, AlertCircle, CheckCircle2, RefreshCw, X, ShieldCheck, Lock, ArrowLeft } from 'lucide-react'
...
export default function OtpVerificationModal({
  ...
  // Handle Step 1 Verification (or Registration verification)
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      const data = err.response?.data
      const msg = data?.detail || data?.error || (typeof data === 'string' ? data : 'Failed to update password. Please try again.')
      setError(msg)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 sm:p-8 overflow-hidden text-gray-900">
        ...
        {/* ── STEP 1: OTP Input (Registration or Password Reset Step 1) ── */}
        {(flow === 'registration' || resetStep === 'otp') ? (
          <div>
            {/* Header */}
          ...
          /* ── STEP 2: Password Reset (Enter New Password) ── */
          <div>
            {/* Header */}
            <div className="text-center mb-6">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-blue-50 border border-blue-100 text-blue-600 mb-3 shadow-sm">
              ...
              <p className="text-sm text-gray-500 mt-1">
                OTP verified! Choose a new password for
              </p>
              <p className="text-sm font-semibold text-gray-800 break-all">
                {email}
              </p>
